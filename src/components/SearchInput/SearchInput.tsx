...
export default function SearchInput() {
  ...
  const searchRequest = (value) => {
    const res = axios.get(`https://test-api-teleshtorm.teleshtorm.org/channels/search?query=${value}&page=0&limit=31`, {headers: {'X-CSRF-Token' : '262f06b948efa287aabe7fa3427186d5a585e27e'}})
  ...
  return (
    <div className={styles.input_container}>
        ...
        <input value={searchPhrase} onChange={handleSearch} placeholder='Поиск...' className={styles.input} type="text" />
        <div ref={dropRef} className={activeMenu ? active : styles.dropdown}>
          {searchResult?.map((item) => (
            ...
            <Link className={styles.dropdown_item} href={'/'}>
              <Image alt='Картинка из поиска' className={styles.dropdown_img} src={item.image}/>
              
              <div className={styles.dropdown_text}>
                <p className={styles.dropdown_name}>{item.name}</p>
                <p className={styles.dropdown_subscribe}><span className={styles.dropdown_number}>{item.subscribers}</span>{item.description}</p>
              </div>
