      ...
      <div className={classNames(styles.overlay, { [styles.active]: open })}>
        <div className={styles.container}>
          ...
          <div className={styles.input_container}>
            <div className={classNames(styles.notification, {[styles.active]: showNotification})}>{t('Ссылка скопирована')}</div>
            <Image onClick={copyToClipboard} className={styles.image} src={'/shareIcon.svg'} width={18} height={20} alt="copy"/>
          <input ref={inputRef} type="text" className={styles.input} value={`https://teleshtorm.org${pathName}`} readOnly />
          </div>
        </div>
      </div>
  );
